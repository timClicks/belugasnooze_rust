...
#[derive(Debug, Error)]
enum EnvError {
    #[error(".env parsing: missing env: '{0}'")]
    NotFound(String),
    #[error(".env parsing: '{0}' - sql file should end '.db'")]
    DbNameInvalid(String),
    #[error(".env parsing: '{0}' - file not found'")]
    FileNotFound(String),
}
...
mod tests {
    ...
    async fn env_parse_offset_ok() {
        ...
        // CHECK
        assert_eq!(result, UtcOffset::from_hms(1, 0, 0).unwrap());
