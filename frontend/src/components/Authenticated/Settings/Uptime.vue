							...
							<div class='text-body-1 text-right' :class='computedColor'>
								<v-tooltip top v-if='item.tooltip'>
									<template v-slot:activator='{ on }'>
										<span v-on='on' class='mono-numbers'>{{ item.value }}</span>
									</template>
...
export default Vue.extend({
	...
	computed: {
		...
		uptimeNameAndValues (): Array<TComputedUptime> {
			return [
				{
					name: 'node uptime',
					tooltip: this.uptimeToString(this.piStatusStore.piNodeUptime),
					value: this.secondsToDays(this.piStatusStore.piNodeUptime)
				},
				{
					name: 'uptime',
	...
	methods: {
		...
		secondsToDays (s: number|undefined): string {
			if (!s) return 'NULL';
			return secondsToText(s, this.$vuetify.breakpoint.smAndDown);
		}
	},
